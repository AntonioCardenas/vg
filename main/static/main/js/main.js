...
var ceiling = JSON.parse(result['message'])['ceil_img'];
var floor = JSON.parse(result['message'])['floor_img'];
var loader = JSON.parse(result['message'])['loader_img'];
var wall = JSON.parse(result['message'])['wall_img'];
var img_1 = JSON.parse(result['message'])['img_1'];
var img_2 = JSON.parse(result['message'])['img_2'];

// Inicia la sesion de metamask.
...
async function getAccount() {
  ...
  web3.eth.getBlockNumber().then( data => getTransactionsByAccount(account, null, data));
}

// Lista los tokens de los que es dueña una cuenta.
...
var Gallery = {
  ...
  boot: function () {
    ...
    Gallery.userBoxMat = new THREE.MeshBasicMaterial({
      color: 0xE06666,
      wireframe: true
    });
  ...
  moveCallback: function (event) {
    if (event = event.touches !== undefined) {} else {}
  ...
  create: function () {
    ...
    Gallery.textureLoader.load(floor, function (texture) {
      texture.wrapS = THREE.RepeatWrapping;
      texture.wrapT = THREE.RepeatWrapping;
      texture.repeat.set(2, 1);

      Gallery.floorMaterial = new THREE.MeshPhongMaterial({
        map: texture
      });
    ...
    Gallery.textureLoader.load(wall,
      function (texture) {
        texture.wrapS = THREE.RepeatWrapping;
        texture.wrapT = THREE.RepeatWrapping;
        texture.repeat.set(2, 1);

        Gallery.wallMaterial = new THREE.MeshLambertMaterial({
          map: texture
        });
    ...
    Gallery.textureLoader.load(ceiling,
      function (texture) {
        texture.wrapS = THREE.RepeatWrapping;
        texture.wrapT = THREE.RepeatWrapping;
        texture.repeat.set(12, 2);

        Gallery.ceilMaterial = new THREE.MeshLambertMaterial({
          map: texture
        });
